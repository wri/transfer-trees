...
def learning_curve_catboost(X_train_all,
    ...
    ax.grid()
    ax.set_xlim([train_sizes.min() - 10000, train_sizes.max() + 10000])
    ax.set_ylim([0.0, 1.2])
    ...
    plt.tight_layout()
    if output_file:
        plt.savefig(output_file + '.pdf', dpi=300, bbox_inches='tight', facecolor='white')
        plt.savefig(output_file + '.svg', format='svg', bbox_inches='tight', facecolor='white')
